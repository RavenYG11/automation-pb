...
- V6 tidak mengubah flow WhatsApp Original yang sudah sukses.
- V6 menambah discovery WhatsApp Dual: package kandidat, launcher icon kedua, lalu user/profile fallback paling akhir.
...
# V6: Discovery Dual. User/profile cuma fallback terakhir, bukan jalur utama.
TRY_DUAL_PACKAGE_DISCOVERY = True
TRY_DUAL_LAUNCHER_ICON_DISCOVERY = True
TRY_DUAL_USER_FALLBACK_LAST = True
TRY_OTHER_NON_OWNER_USERS_FOR_DUAL = True
DUAL_DISCOVERY_MAX_ICON_CANDIDATES = 8
DUAL_DISCOVERY_APP_DRAWER_PAGES = 4

# Kalau launcher icon Dual tidak ada di Home/App Drawer dump pertama, script akan coba app drawer.
# Posisi ini aman untuk Samsung launcher default: swipe up dari Home.
...
# ============================================================
# V6 DUAL DISCOVERY HELPERS
# ============================================================

WHATSAPP_RELATED_PACKAGE_KEYWORDS = [
    "whatsapp", "w4b", "wa", "dual", "clone", "parallel", "messenger"
]


def save_shell_output(debug_dir: Path, filename: str, shell_args: List[str], timeout: int = 25) -> str:
    """Jalankan adb shell command dan simpan output ke debug."""
    ensure_dir(debug_dir)
    res = adb_shell(shell_args, timeout=timeout)
    text = res.combined
    write_text(debug_dir / filename, text)
    return text


def parse_packages_from_pm_output(text: str) -> List[str]:
    """Parse package dari output pm list packages / pm list packages -f."""
    packages: List[str] = []
    for line in (text or "").splitlines():
        ...
        if "=" in line:
            ...
            if ":" in pkg and "/" not in pkg:
                # jarang, tapi jangan rusak output aneh
                pkg = pkg.split(":")[-1].strip()
        else:
            continue
        if pkg and re.match(r"^[a-zA-Z0-9_.]+$", pkg) and pkg not in packages:
            packages.append(pkg)
    return packages


def discover_whatsapp_related_packages(debug_dir: Path) -> List[str]:
    """
    V6: Cari semua package yang mungkin terkait WA/Dual/Clone.
    Ini discovery, bukan kesimpulan. Samsung Dual sering tetap com.whatsapp,
    tapi kalau ternyata clone package berbeda, ini yang akan menangkap.
    """
    ensure_dir(debug_dir)
    outputs: List[str] = []

    commands = {
        "pm_list_packages_f_u.txt": ["pm", "list", "packages", "-f", "-u"],
        "pm_list_packages_all_default.txt": ["pm", "list", "packages"],
    }

    users = get_android_users()
    for user_id in users:
        commands[f"pm_list_packages_f_user_{user_id}.txt"] = ["pm", "list", "packages", "-f", "--user", str(user_id)]
        commands[f"pm_list_packages_user_{user_id}.txt"] = ["pm", "list", "packages", "--user", str(user_id)]

    for filename, cmd in commands.items():
        outputs.append(save_shell_output(debug_dir, filename, cmd, timeout=35))

    found: List[str] = []
    for out in outputs:
        for pkg in parse_packages_from_pm_output(out):
            low = pkg.lower()
            # Utamakan package yang jelas-jelas WhatsApp; keyword lain disimpan di raw debug saja.
            if "whatsapp" in low or "w4b" in low:
                if pkg not in found:
                    found.append(pkg)

    # Simpan juga kandidat longgar buat inspeksi manual, tapi jangan otomatis dibuka semua.
    loose: List[str] = []
    for out in outputs:
        for pkg in parse_packages_from_pm_output(out):
            low = pkg.lower()
            if any(k in low for k in WHATSAPP_RELATED_PACKAGE_KEYWORDS):
                if pkg not in loose:
                    loose.append(pkg)

    write_text(debug_dir / "discovered_whatsapp_packages.json", json.dumps({
        "strict_whatsapp_packages": found,
        "loose_related_packages": loose,
        "note": "Strict dibuka otomatis kalau bukan com.whatsapp/com.whatsapp.w4b. Loose cuma bukti debug supaya package Samsung/clone bisa dianalisis.",
    }, indent=4, ensure_ascii=False))

    return found


def launcher_candidate_key(candidate: Dict[str, Any]) -> Tuple[str, int, str, str, str]:
    return (
        str(candidate.get("source", "")),
        int(candidate.get("page", 0)),
        str(candidate.get("bounds_raw", "")),
        normalized_text(candidate.get("text", "")),
        normalized_text(candidate.get("desc", "")),
    )


def find_whatsapp_launcher_candidates(xml_path: Optional[Path], source: str, page: int) -> List[Dict[str, Any]]:
    """Cari icon/label WhatsApp di XML Home/App Drawer."""
    if not xml_path:
        return []
    ...
    for n in nodes:
        ...
        desc = n.get("desc", "") or ""
        rid = n.get("rid", "") or ""
        blob = " ".join([text, desc, rid]).lower()

        if "whatsapp" not in blob:
            continue
        if "business" in blob or "w4b" in blob:
            continue

        # Hindari text dari notifikasi/chat kalau Home kebetulan ada widget/notif.
        x1, y1, x2, y2 = b
        w = x2 - x1
        h = y2 - y1
        if w < 20 or h < 15:
...
def collect_launcher_whatsapp_candidates(debug_dir: Path) -> List[Dict[str, Any]]:
    """
    V6: Cari icon WhatsApp di Home dan App Drawer.
    Kalau Samsung Dual Messenger muncul sebagai icon kedua, harusnya ketemu di sini.
    """
    ensure_dir(debug_dir)
    width, height = get_screen_size()
    candidates: List[Dict[str, Any]] = []

    log("[DUAL-DISCOVERY] Cari icon WhatsApp di Home.")
    press_home()
    sleep(WAIT_MEDIUM)
    save_screenshot(debug_dir, "launcher_home.png")
    home_xml = dump_ui(debug_dir, "launcher_home.xml")
    candidates.extend(find_whatsapp_launcher_candidates(home_xml, "home", 0))

    log("[DUAL-DISCOVERY] Buka App Drawer dan cari icon WhatsApp.")
    press_home()
    sleep(WAIT_SHORT)
    swipe(int(width * 0.50), int(height * 0.86), int(width * 0.50), int(height * 0.20), 450)
    sleep(WAIT_LONG)

    for page in range(DUAL_DISCOVERY_APP_DRAWER_PAGES):
        save_screenshot(debug_dir, f"launcher_appdrawer_page_{page}.png")
        xml = dump_ui(debug_dir, f"launcher_appdrawer_page_{page}.xml")
        candidates.extend(find_whatsapp_launcher_candidates(xml, "app_drawer", page))

        # Samsung app drawer kadang horizontal per halaman. Kalau ternyata vertical, swipe ini biasanya tidak fatal.
        swipe(int(width * 0.82), int(height * 0.55), int(width * 0.18), int(height * 0.55), 350)
        sleep(WAIT_MEDIUM)
    ...
    write_text(debug_dir / "launcher_whatsapp_candidates.json", json.dumps(out, indent=4, ensure_ascii=False, default=str))
    log(f"[DUAL-DISCOVERY] Kandidat icon WhatsApp ditemukan: {len(out)}")
    return out


def restore_launcher_position_for_candidate(candidate: Dict[str, Any], debug_dir: Path, prefix: str) -> None:
    """Balik ke posisi launcher sesuai kandidat sebelum tap bounds."""
    width, height = get_screen_size()
    ...
    save_screenshot(debug_dir, f"{prefix}_before_icon_tap.png")
    dump_ui(debug_dir, f"{prefix}_before_icon_tap.xml")


def tap_launcher_candidate(candidate: Dict[str, Any]) -> None:
    """Tap kandidat icon/label. Kalau yang kedetect label, tap center label lalu area sedikit di atasnya."""
    b = candidate.get("bounds")
    if not b:
        return
    x1, y1, x2, y2 = b
    ...
    # Kalau label text terlalu bawah dan app belum kebuka, tap area icon di atas label.
    if y2 - y1 < 80:
        tap(cx, max(40, cy - 90))
        sleep(WAIT_LONG)


def scan_current_open_personal_whatsapp(
    package: str,
    label_hint: str,
    debug_dir: Path,
    launch_user: Optional[str] = None,
    launch_method: str = "already_opened",
) -> Dict[str, Any]:
    """
    Scan WA yang sudah terbuka dari launcher icon/package discovery.
    Ini sengaja tidak memanggil open_package(), supaya flow Original v4 tetap tidak berubah.
    """
    result = base_result(label_hint, package)
    result["launch_user"] = launch_user
    result["package_found"] = True
    result["notes"].append(f"Launch method: {launch_method}")

    try:
        opened = verify_package_opened(package, debug_dir=debug_dir, tag=launch_method)
        ...
        if not opened:
            ...
            result["notes"].append("Setelah discovery click/launch, package belum jadi top focused app.")
        ...
        # Kalau app terbuka dari chooser/link dan bukan di home chat list, coba back ringan dulu.
        # Ini hanya untuk discovery Dual. Kalau sudah home chat list, back biasanya tidak fatal.
        for idx in range(2):
            tmp_xml = dump_ui(debug_dir, f"pre_settings_state_{idx}.xml")
            tmp_nodes = parse_ui_xml(tmp_xml) if tmp_xml else []
            blob = " | ".join(get_all_texts(tmp_nodes)).lower()
            if any(k in blob for k in SETTINGS_KEYWORDS) or "chats" in blob or "chat" in blob or "pembaruan" in blob or "updates" in blob:
                break
            # Jangan back kalau ada indikasi sudah di Settings/Profile.
            if "settings" in blob or "pengaturan" in blob:
                break
            press_back()
            sleep(WAIT_MEDIUM)
        ...
        if not settings_xml:
            ...
            result["notes"].append("Gagal masuk Settings/Pengaturan setelah app dibuka via discovery.")
            save_result_json(result, debug_dir)
            return result
        ...
        if accounts:
            result["status"] = "success" if detected_user is not None else "package_found_user_unknown"
...
def result_has_different_accounts(result: Dict[str, Any], original_accounts: set) -> bool:
    acc = account_key_set(result)
    return bool(acc) and acc != original_accounts


def try_dual_by_package_discovery(original_accounts: set, discovery_dir: Path, original_package: str) -> Optional[Dict[str, Any]]:
    """Coba package kandidat yang bukan com.whatsapp/com.whatsapp.w4b."""
    if not TRY_DUAL_PACKAGE_DISCOVERY:
        return None

    packages = discover_whatsapp_related_packages(discovery_dir)
    candidate_packages = [p for p in packages if p not in {original_package, "com.whatsapp.w4b"}]
    write_text(discovery_dir / "package_open_candidates.json", json.dumps(candidate_packages, indent=4, ensure_ascii=False))

    for pkg in candidate_packages:
        log(f"[DUAL-DISCOVERY] Coba package kandidat: {pkg}")
        result = scan_personal_whatsapp(pkg, "WhatsApp Dual / Clone", None)
        if result_has_different_accounts(result, original_accounts):
            result["final_label"] = "WhatsApp Dual / Clone"
            result["notes"].append(f"Ditemukan via package discovery: {pkg}")
            return result
        log(f"[DUAL-DISCOVERY] Package {pkg} belum terbukti clone / akun sama / gagal baca.")

    return None


def try_dual_by_launcher_icon_discovery(original_accounts: set, discovery_dir: Path, package: str) -> Optional[Dict[str, Any]]:
    """Coba buka WhatsApp dari icon Home/App Drawer, cocok untuk Samsung Dual Messenger."""
    if not TRY_DUAL_LAUNCHER_ICON_DISCOVERY:
        return None

    candidates = collect_launcher_whatsapp_candidates(discovery_dir)
    if not candidates:
        return None

    # Kalau ada lebih dari 1 icon, coba kandidat yang bukan urutan pertama dulu.
    # Urutan pertama sering Original. Setelah itu baru coba first sebagai fallback pembuktian.
    ordered = candidates[1:] + candidates[:1] if len(candidates) > 1 else candidates
    ordered = ordered[:DUAL_DISCOVERY_MAX_ICON_CANDIDATES]

    attempts_summary: List[Dict[str, Any]] = []

    for idx, candidate in enumerate(ordered, start=1):
        text = candidate.get("text") or candidate.get("desc") or "WhatsApp"
        log(f"[DUAL-DISCOVERY] Coba icon kandidat #{idx}: {text} {candidate.get('bounds_raw')}")

        debug_dir = create_debug_dir(f"wa_dual_icon_candidate_{idx}", None)
        write_text(debug_dir / "candidate.json", json.dumps(candidate, indent=4, ensure_ascii=False, default=str))

        restore_launcher_position_for_candidate(candidate, debug_dir, prefix=f"candidate_{idx}")
        tap_launcher_candidate(candidate)
        sleep(WAIT_LONG)

        result = scan_current_open_personal_whatsapp(
            package=package,
            label_hint="WhatsApp Dual / Clone",
            debug_dir=debug_dir,
            launch_user=None,
            launch_method=f"launcher_icon_candidate_{idx}",
        )
        ...
        if result_has_different_accounts(result, original_accounts):
            result["final_label"] = "WhatsApp Dual / Clone"
            result["notes"].append("Ditemukan via launcher icon discovery. Kemungkinan besar icon Samsung Dual Messenger.")
            write_text(discovery_dir / "launcher_icon_attempts_summary.json", json.dumps(attempts_summary, indent=4, ensure_ascii=False, default=str))
            return result

        if account_key_set(result) == original_accounts and account_key_set(result):
            log("[DUAL-DISCOVERY] Icon ini membuka akun yang sama dengan Original, lanjut kandidat lain.")
        else:
            log("[DUAL-DISCOVERY] Icon ini belum menghasilkan akun clone yang bisa dibaca.")

    write_text(discovery_dir / "launcher_icon_attempts_summary.json", json.dumps(attempts_summary, indent=4, ensure_ascii=False, default=str))
    return None


def try_dual_by_user_fallback_last(original_accounts: set, discovery_dir: Path, package: str) -> Optional[Dict[str, Any]]:
    """Fallback terakhir: coba user/profile. Ini bukan validasi utama."""
    if not TRY_DUAL_USER_FALLBACK_LAST:
        return None

    users = collect_dual_diagnostics(discovery_dir, package)
    ...
    for dual_user in candidate_users:
        log(f"[DUAL-DISCOVERY] Fallback terakhir via user/profile {dual_user}.")
        dual = scan_personal_whatsapp(package, "WhatsApp Dual / Clone", dual_user)
        if result_has_different_accounts(dual, original_accounts):
            dual["final_label"] = "WhatsApp Dual / Clone"
            dual["notes"].append(f"Ditemukan via user/profile fallback: {dual_user}")
            return dual
        if account_key_set(dual) == original_accounts and account_key_set(dual):
            log(f"[DUAL-DISCOVERY] User/profile {dual_user} membuka akun Original yang sama.")
        else:
            log(f"[DUAL-DISCOVERY] User/profile {dual_user} gagal membuka/membaca clone.")
...
def scan_all_personal_instances(target: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    V6 Personal scan:
    - Original tetap pakai flow v4 yang sudah terbukti sukses.
    - Dual tidak diasumsikan user 95.
    - Dual dicari lewat discovery:
        1) package kandidat WhatsApp/clone,
        2) launcher icon Home/App Drawer,
        3) user/profile fallback terakhir.
    """
    ...
    # 1) Original: JANGAN DIUBAH. Ini flow v4 yang di HP lo sudah sukses.
    original = scan_personal_whatsapp(package, "WhatsApp Original", None)
    results.append(original)
    original_accounts = account_key_set(original)

    # 2) Dual Discovery: fokus cari app/icon/instance yang benar-benar bisa dibuka.
    discovery_dir = create_debug_dir("wa_dual_discovery", None)
    write_text(discovery_dir / "original_accounts.json", json.dumps(sorted(list(original_accounts)), indent=4, ensure_ascii=False))

    found_dual: Optional[Dict[str, Any]] = None
    discovery_notes: List[str] = []

    # 2A. Coba kalau ternyata clone punya package beda.
    try:
        found_dual = try_dual_by_package_discovery(original_accounts, discovery_dir, package)
    except Exception as e:
        discovery_notes.append(f"Package discovery error: {type(e).__name__}: {e}")
        write_text(discovery_dir / "package_discovery_error.txt", discovery_notes[-1])

    # 2B. Coba launcher icon kedua. Ini target utama untuk Samsung Dual Messenger.
    if not found_dual:
        try:
            found_dual = try_dual_by_launcher_icon_discovery(original_accounts, discovery_dir, package)
        except Exception as e:
            discovery_notes.append(f"Launcher icon discovery error: {type(e).__name__}: {e}")
            write_text(discovery_dir / "launcher_icon_discovery_error.txt", discovery_notes[-1])

    # 2C. User/profile fallback paling akhir, bukan validasi awal.
    if not found_dual:
        try:
            found_dual = try_dual_by_user_fallback_last(original_accounts, discovery_dir, package)
        except Exception as e:
            discovery_notes.append(f"User/profile fallback error: {type(e).__name__}: {e}")
            write_text(discovery_dir / "user_profile_fallback_error.txt", discovery_notes[-1])

    if found_dual:
        found_dual["final_label"] = "WhatsApp Dual / Clone"
        found_dual.setdefault("notes", []).append(f"Dual discovery dir: {discovery_dir}")
        results.append(found_dual)
    else:
        reason = "Tidak ada bukti instance Dual/Clone yang berhasil dibuka atau terbaca lewat package discovery, launcher icon discovery, maupun fallback user/profile."
        reason += f" Discovery: {discovery_dir}"
        if discovery_notes:
            reason += " | " + " | ".join(discovery_notes)
